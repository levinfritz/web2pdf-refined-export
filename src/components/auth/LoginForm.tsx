...
import { Loader2, Eye, EyeOff, Github, AlertCircle } from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";
import { motion } from "framer-motion";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from "@/components/ui/dialog";
import { toast } from "sonner";

// Verbesserte Validierungsregeln mit detaillierteren Fehlermeldungen
...
type LoginFormValues = z.infer<typeof loginSchema>;
  ...
  // Hauptformular-Rendering-Funktion
      ...
      <motion.form onSubmit={handleSubmit(onSubmit)} className="space-y-4" variants={itemVariants}>
        ...
        >
          {isLoading && !socialLoading ? (
            ...
            <>
              <span className="transition-transform group-hover:translate-y-[-1px]">Anmelden</span>
            </>
          )}
        </Button>
      </motion.form>
...
const LoginForm: React.FC<{ onToggle: () => void }> = ({ onToggle }) => {
  const { login, loginWithGoogle, loginWithGitHub, isLoading, resetPassword } = useAuth();
